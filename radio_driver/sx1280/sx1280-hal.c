...
{
    HAL_Delay( 20 );
    GpioWrite( RADIO_RESET_GPIO_Port, RADIO_RESET_Pin, 0 );
    HAL_Delay( 50 );
    GpioWrite( RADIO_RESET_GPIO_Port, RADIO_RESET_Pin, 1 );
    HAL_Delay( 20 );
}
...
void SX1280HalClearInstructionRam( void )
{
    ...
    GpioWrite( RADIO_NSS_GPIO_Port, RADIO_NSS_Pin, 0 );

    SpiIn( halTxBuffer, halSize );

    GpioWrite( RADIO_NSS_GPIO_Port, RADIO_NSS_Pin, 1 );
...
{
	uint8_t Status = GpioRead( RADIO_BUSY_GPIO_Port, RADIO_BUSY_Pin );
